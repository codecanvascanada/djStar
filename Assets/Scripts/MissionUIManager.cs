...
public class MissionUIManager : MonoBehaviour
{
    ...
    public void RefreshMissionUI()
    {
        ...
        {
            Debug.LogError("MissionManager instance not found! Cannot refresh UI.");
            return;
        }
        ...
        {
            Debug.LogError("MissionItemPrefab is NOT ASSIGNED in the MissionUIManager inspector!");
            return;
        }
        MissionItemUI prefabScript = missionItemPrefab.GetComponent<MissionItemUI>();
        if (prefabScript == null)
        {
            Debug.LogError("PREFAB ITSELF is missing the MissionItemUI.cs script!");
            return;
        }
        if (prefabScript.descriptionText == null)
        {
            Debug.LogError("PREFAB's 'Description Text' field is NOT ASSIGNED in its inspector!");
            return;
        }
        Debug.Log($"Found {MissionManager.instance.currentDailyMissions.Count} missions to display.");
        ...
        foreach (var mission in MissionManager.instance.currentDailyMissions)
        {
            ...
            if (itemUI == null)
            {
                ...
                Debug.LogError("Instantiated MissionItemPrefab is MISSING the MissionItemUI.cs script! Please check the prefab.", itemGO);
                continue; 
            }
    ...
    private void ClaimMissionReward(MissionManager.Mission mission)
    {
        ...
        Debug.Log($"Attempting to claim reward for mission: {mission.description}");
    ...
    private void ClaimAllMissionsBonus()
    {
        ...
        if (allCompleted && allMissionsCompleteButton.interactable)
        {
            ...
            Debug.Log("Claiming all missions complete bonus.");
            allMissionsCompleteButton.interactable = false; // Disable after claiming
            RefreshMissionUI(); // Refresh UI to show updated status
        }
        else
        {
            Debug.Log("Not all missions are completed yet, or bonus already claimed.");
        }
    }
