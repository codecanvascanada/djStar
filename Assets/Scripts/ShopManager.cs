...
public class ShopManager : MonoBehaviour
{
    ...
    public void BuyGemPack(string packId)
    {
        ...
        if (pack != null)
        {
            ...
            UnityEngine.Debug.Log(string.Format("Simulating purchase of {0} for {1}.", pack.id, pack.priceString));
            GemManager.instance.AddGems(pack.gemsAmount + pack.bonusGems);
            UnityEngine.Debug.Log(string.Format("Successfully purchased {0} Gems!", pack.gemsAmount + pack.bonusGems));
            // Here you would typically trigger a UI update for Gem balance
        }
        else
        {
            UnityEngine.Debug.LogError(string.Format("Gem pack with ID '{0}' not found.", packId));
        }
    }
    ...
    public void BuyCoinPack(string packId)
    {
        ...
        if (pack != null)
        {
            if (GemManager.instance.SpendGems(pack.gemCost))
            {
                ...
                UnityEngine.Debug.Log(string.Format("Successfully purchased {0} Coins for {1} Gems!", pack.coinsAmount, pack.gemCost));
                // Here you would typically trigger a UI update for Coin and Gem balances
            }
            else
            {
                UnityEngine.Debug.Log("Not enough Gems to buy this Coin pack.");
                // Here you would typically show a UI message to the user
            }
        }
        else
        {
            UnityEngine.Debug.LogError(string.Format("Coin pack with ID '{0}' not found.", packId));
        }
    }
