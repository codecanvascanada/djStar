...
public class StageSelectManager : MonoBehaviour
{
    ...
    public void OnSongSelected(string songId)
    {
        ...
        Action<SongInfo> onComplete = (loadedSongInfo) => {
            ...
            {
                // if (showDebugLogs) UnityEngine.Debug.Log(string.Format("Assets for '{0}' are ready. Starting game...", songId));
                StartCoroutine(PlaySongAndFade(loadedSongInfo));
            }
            else
            {
                // if (showDebugLogs) UnityEngine.Debug.LogError(string.Format("Failed to prepare assets for song '{0}'.", songId));
